...
const Layout: React.FC<LayoutProps> = ({ children, onAdminClick }) => {
  return (
    <div className="min-h-screen w-full relative overflow-hidden flex flex-col items-center justify-center p-4">
      ...
      {/* Main Content Wrapper - z-10 to stay above background, w-full to allow children to control width */}
      <div className="relative z-10 w-full flex flex-col items-center justify-center">
        {children}
      </div>
      
      {/* Footer - z-50 to ensure visibility on top of all content */}
      <footer className="fixed bottom-4 text-xs text-gray-500 z-50 text-center w-full flex items-center justify-center gap-2 pointer-events-none">
        <span className="pointer-events-auto">Powered by Google Gemini • For Entertainment Purposes Only</span>
        {onAdminClick && (
          <button 
            onClick={onAdminClick}
            className="opacity-50 hover:opacity-100 transition-opacity p-2 pointer-events-auto text-gray-400 hover:text-white"
            title="Admin Access"
          >
